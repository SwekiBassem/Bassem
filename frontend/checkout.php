...
include "C:/wamp64/www/tajriba/entites/livraison.php";

//=$row['num_com']

$sql="SELECT * FROM commande ORDER BY num_com DESC LIMIT 1";
$db = config::getConnexion();
$liste=$db->query($sql);


	
if (isset($_POST['nom']) and isset($_POST['adresse']) and isset($_POST['numtel']) and isset($_POST['liv'])and isset($_POST['date_livraison'])){
$livraison1=new livraison($_POST['idclient'],$_POST['nom'],$_POST['adresse'],$_POST['numtel'] ,$_POST['numcommande'],$_POST['liv'],$_POST['date_livraison']);
//Partie2
/*
var_dump($livraison1);
}
...
{
    echo "<script>alert(\"verifier vos donnĂ©es\")</script>";
}	
}else{
	echo "";
}
...
<h3 class="mb-4 billing-heading">Livraison</h3>
	          	<div class="row align-items-end">
					...
					<div class="form-group has-warning">
                        <label class="control-label" for="warning">Date livraison</label>
                        <input type="Date" class="form-control" id="warning" name="date_livraison"/>
                    </div>
                    <div class="form-group">
                        <label>numero commande</label>
                        <select class="form-control" name="numcommande" type="text">
                        <?php
						foreach($liste as $row ){
							?>
                        <option><?PHP echo $row['num_com'] ?></option>   
<?php
}
?>	
